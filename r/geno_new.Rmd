...
number_table<- int_table %>% 
  mutate(start=V2, 
         ...
         num_pick=round(weight*sum(counts)*0.7))%>%
  arrange(desc(length)) %>%
  filter(num_pick>1)
number_table[,-c(1,2)]
}
...
# NOTE: Here we eliminate all the short rows
...
```{r}
#### write a for loop to put all the output of the function into a list and then unlist it to get the vector that contains all the random starting position. 

start <- function(count_table){
  interval_start <- list()
  for (i in 1:nrow(count_table)){
  interval_start[[i]]<-get_start_pt(count_table,i)
}
   unlist(interval_start)
}

random_start_Y<-start(count_Y)
random_start_X<-start(count_X)
random_start_L2<-start(count_L2)
random_start_R2<-start(count_R2)
random_start_L3<-start(count_L3)
```


```{r}
# use the starting positions to get the intervals and put them into a dataframe
random_df <- function(random_start_list){
  random_start_l <- sample(random_start_list, length(random_start_list), replace=FALSE)
  data.frame(start = random_start_l, end = random_start_l + 2000)
} 

random_int_df_Y <- random_df(random_start_Y)
random_int_df_X <- random_df(random_start_X)
random_int_df_L2 <- random_df(random_start_L2)
random_int_df_R2 <- random_df(random_start_R2)
random_int_df_L3 <- random_df(random_start_L3)

#save(random_int_df_Y,file = "C:/Users/Yao/Desktop/sp18/DNA/random_int_df_Y.csv")
#save(random_int_df_X,file = "C:/Users/Yao/Desktop/sp18/DNA/random_int_df_X.csv")
#save(random_int_df_L2,file = "C:/Users/Yao/Desktop/sp18/DNA/random_int_df_L2.csv")
#save(random_int_df_R2,file = "C:/Users/Yao/Desktop/sp18/DNA/random_int_df_R2.csv")
#save(random_int_df_L3,file = "C:/Users/Yao/Desktop/sp18/DNA/random_int_df_L3.csv")
